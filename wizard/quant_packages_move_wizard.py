...
class StockQuantPackageMove(models.TransientModel):
    ...
    def do_detailed_transfer(self):
        ...
        for item in self.pack_move_items:
            # localizacion destino y origen del producto se revisa si no son iguales
            if item.dest_loc is not item.source_loc:
                # se realiza el movimiento de cada uno de los productos con el metodo move_to
                for quant in item.package.quant_ids:
                    # este metodo esta en el modelo stock.quant por que quant es un objeto d edicho modelo
